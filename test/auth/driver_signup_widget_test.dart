...
void main() {
  testWidgets('DriverSignUp navigates 3-step workflow and enforces vehicle field validation', (tester) async {
    await tester.binding.setSurfaceSize(const Size(1200, 2400));
    await tester.pumpWidget(
      const MaterialApp(
        home: Scaffold(
          body: SingleChildScrollView(child: DriverSignUp()),
        ),
      ),
    );
    ...
    await tester.enterText(find.widgetWithText(TextFormField, 'Email'), 'driver@buenatoda.com');
    await tester.enterText(find.widgetWithText(TextFormField, 'First Name'), 'Jose');
    await tester.enterText(find.widgetWithText(TextFormField, 'Last Name'), 'Rizal');
    ...
    await tester.pumpAndSettle();
    await tester.enterText(find.widgetWithText(TextFormField, 'Password'), 'driverPass123');
    await tester.enterText(find.widgetWithText(TextFormField, 'Confirm Password'), 'driverPass123');
    await tester.pumpAndSettle();
    ...
    expect(find.widgetWithText(TextFormField, 'Tricycle Color'), findsOneWidget);
    expect(find.widgetWithText(TextFormField, 'Plate / Franchise No.'), findsOneWidget);
    ...
    expect(find.text('Vehicle Information & Photos'), findsOneWidget);
    expect(find.text('Please complete all required vehicle text fields.'), findsOneWidget);
  });
}
